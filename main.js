...
function filter_Links(bilateral_links, nodeMap) {
  ...
  bilateral_links.forEach(link => {
    if (!linkMap.get(link.target) && !linkMap.get(link.source)) { // wenn kein Eintrag für target und kein Eintrag für source
      linkMap.set(link.source, [link.target]); // neuen Eintrag für source anlegen
    } else if (!linkMap.get(link.target) && linkMap.get(link.source)) { // wenn kein Eintrag für target, aber ein Eintrag für source
    ...
    } else if (!linkMap.get(link.target).includes(link.source)) { // wenn Eintrag für target die source nicht enthält 
      linkMap.set(link.source, linkMap.get(link.source).concat(link.target)); // target an Eintrag für source hinzufügen
    }
  });
...
function dragstarted(d) {
  // wenn kein vorheriges Drag-Event aktiv 
  if (d3.event.active == 0) {
    // erlaube Knoten sich zu bewegen
...
function generate_Info(graph) {
  ...
  /* if (graph.information.graphic_type) {
    information_text += graph.information.graphic_type;
  }
